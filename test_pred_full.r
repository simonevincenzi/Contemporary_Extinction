...
acc_table_full_df = data.frame(model = c("GLM", "GAM", "RF"), accuracy = NA, false_pos = NA, false_neg = NA) # table with accuracy measures

ext.lrm.full = readRDS("ext.lrm.full.RDS")
...
acc_table_full_df$accuracy[which(acc_table_full_df$model == "GLM")] = accuracy.lrm

false.pos.val.lrm = val.table.lrm[2,1]/(val.table.lrm[2,1]+val.table.lrm[1,1]) #false positives for the test dataset

acc_table_full_df$false_pos[which(acc_table_full_df$model == "GLM")] = false.pos.val.lrm

false.neg.val.lrm = val.table.lrm[1,2]/(val.table.lrm[1,2]+val.table.lrm[2,2])

acc_table_full_df$false_neg[which(acc_table_full_df$model == "GLM")] = false.neg.val.lrm
...
# legend(0.2,.5,col=c("gray30","gray50","gray70","gray90"),lwd=rep(3,4),c("Sensitivity","Specificity","Classification Rate","Distance"),box.lwd = 0,box.col = "white",bg = "white")
# 
# saveRDS(cutoff, "cutoff_gam.RDS")

cutoff = readRDS("cutoff_gam_full.RDS")
...
acc_table_full_df$accuracy[which(acc_table_full_df$model == "GAM")] = accuracy.gam

false.pos.val.gam = val.table.gam[2,1]/(val.table.gam[2,1]+val.table.gam[1,1]) #false positives for the test dataset

acc_table_full_df$false_pos[which(acc_table_full_df$model == "GAM")] = false.pos.val.gam

false.neg.val.gam = val.table.gam[1,2]/(val.table.gam[1,2]+val.table.gam[2,2])

acc_table_full_df$false_neg[which(acc_table_full_df$model == "GAM")] = false.neg.val.gam
...
acc_table_full_df$accuracy[which(acc_table_full_df$model == "RF")] = accuracy.rf

false.pos.val.rf = val.table.rf[2,1]/(val.table.rf[2,1]+val.table.rf[1,1]) #false positives for the validation dataset

acc_table_full_df$false_pos[which(acc_table_full_df$model == "RF")] = false.pos.val.rf

false.neg.val.rf = val.table.rf[1,2]/(val.table.rf[1,2]+val.table.rf[2,2])

acc_table_full_df$false_neg[which(acc_table_full_df$model == "RF")] = false.neg.val.rf


saveRDS(acc_table_full_df,"acc_table_full_df.RDS")
